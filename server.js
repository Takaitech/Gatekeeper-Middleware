...
function gateKeeper(req, res, next) {
  let userInfo = req.get("x-username-and-password") ;
  let userObject = querystring.parse(userInfo,'&','=');
  console.log(userObject);
  for(let i=0; i< USERS.length; i++) {
    if (userObject.user === USERS[i].userName & userObject.pass === USERS[i].password) {
      console.log(USERS[i].id);
      let matchedUser = USERS[i];
